...
func main() {
	...
	timber.Donef("app token: %s", appToken)
	timber.Donef("expires: %s", expiration.Format("January 2 2006"))
	...
	waitGroup.Add(1)
	timber.Infof("starting server for user token at %s", url)
	go func() {
	...
	if err != nil {
		timber.Fatalf(err, "failed to open url in browser: %s", url)
	}
	waitGroup.Wait()
}
